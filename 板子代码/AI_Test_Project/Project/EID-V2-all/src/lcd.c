...
u16 LCD_ReadPoint(u16 x,u16 y)
{
	...
	}else if(lcddev.id==0X6804)
	{
		LCD_RD_CLR;					   
	 	LCD_RD_SET;
		r=DATAIN();//6804µÚ¶þ´Î¶ÁÈ¡µÄ²ÅÊÇÕæÊµÖµ 
	}	 
	LCD_CS_SET;
	DATAOUT(0xFFFF);  //È«²¿Êä³ö¸ß
...
{	   
	...
	LCD_RS_CLR;
 	LCD_CS_CLR; 
	DATAOUT(lcddev.wramcmd);//Ð´Ö¸Áî  
	LCD_WR_CLR; 
	LCD_WR_SET; 
 	LCD_CS_SET; 
	LCD_WR_DATA(color);		//Ð´Êý¾Ý
}
//SSD1963 ±³¹âÉèÖÃ
...
//³õÊ¼»¯lcd
//¸Ã³õÊ¼»¯º¯Êý¿ÉÒÔ³õÊ¼»¯¸÷ÖÖALIENTEK³öÆ·µÄLCDÒº¾§ÆÁ
//±¾º¯ÊýÕ¼ÓÃ½Ï´óflash,ÓÃ»§¿ÉÒÔ¸ù¾Ý×Ô¼ºµÄÊµ¼ÊÇé¿ö,É¾µôÎ´ÓÃµ½µÄLCD³õÊ¼»¯´úÂë.ÒÔ½ÚÊ¡¿Õ¼ä.
void LCD_Init(void)
{ 
...
//	lcddev.id=0X9341;
	...
	}else if(lcddev.id==0X1963)
	{
		...
		LCD_WR_REG(0xE6);		//ÉèÖÃÏñËØÆµÂÊ,33Mhz
		LCD_WR_DATA(0x2F);
		LCD_WR_DATA(0xFF);
		LCD_WR_DATA(0xFF);
...
{  
 	...
 	for(i=0;i<height;i++)
	{
		...
		LCD_WriteRAM_Prepare();     //¿ªÊ¼Ð´ÈëGRAM
		for(j=0;j<width;j++)LCD_WR_DATA(color[i*width+j]);//Ð´ÈëÊý¾Ý 
	}	  
}

void LCD_ClearFullColor_Circle(int x, int y, int r)
{  
	u16 i,j;
	for(i = x - r; i < x + r; i ++) {
		for(j = y - r; j < y + r; j ++) {
			if((i-x) * (i-x) + (j-y) * (j-y) < r * r && i != x && j != y) {
				LCD_SetCursor(i,j);   	//ÉèÖÃ¹â±êÎ»ÖÃ 
				LCD_WriteRAM_Prepare();     //¿ªÊ¼Ð´ÈëGRAM
				LCD_WR_DATA(YELLOW);//Ð´ÈëÊý¾Ý 
			}
		}
	}
	/*
	for(i = x - r; i <= x + r; i ++) {
		for(j = y - r; j <= y + r; j ++) {
			if((x - i) * (x - i) + (y - j) * (y - j) <= r * r && x != i && y != j) {
				LCD_SetCursor(i, j);
...
void LCD_ClearColor_Circle(u16 x,u16 y,u8 r)
{
	u16 i,j;
	for(i = x - r; i < x + r; i ++) {
		for(j = y - r; j < y + r; j ++) {
			...
			else if((i-x) * (i-x) + (j-y) * (j-y) < r * r && (i == x || y == j)) {
				LCD_SetCursor(i,j);   	//ÉèÖÃ¹â±êÎ»ÖÃ 
				LCD_WriteRAM_Prepare();     //¿ªÊ¼Ð´ÈëGRAM
				LCD_WR_DATA(BLACK);//Ð´ÈëÊý¾
		}
		}
	}
	/*
...
void LCD_Draw_Circle_Full(u16 x,u16 y,u16 r) {
	u16 i;
	u16 j;
	for(i = x - r; i < x + r; i ++) {
		for(j = y - r; j < y + r; j ++) {
...
void LCD_Draw_Circle_Empty(u16 x,u16 y,u16 r) {
	u16 i;
	u16 j;
	for(i = x - r; i < x + r; i ++) {
		for(j = y - r; j < y + r; j ++) {
...
#define	T_CS_SET  GPIOA->BSRR=1<<15    //Æ¬Ñ¡¶Ë¿Ú  		PA15
#define	T_WR_SET	GPIOB->BSRR=1<<5    //Ð´Êý¾ÝMOSI			PB5
#define	T_SCK_SET	GPIOA->BSRR=1<<5    //SCK			PA5
								    
#define	T_CS_CLR  GPIOA->BRR=1<<15     //Æ¬Ñ¡¶Ë¿Ú  		PA15
#define	T_WR_CLR	GPIOB->BRR=1<<5     //Ð´Êý¾Ý	MOSI		PB5
#define	T_SCK_CLR	GPIOA->BRR=1<<5     //SCK			PA5  

#define	T_IN_STATUE	((GPIOA->IDR & 0X40)?1:0)    //Êý¾ÝMISO PA6	   
...
//     GPIO_ResetBits(GPIOA,GPIO_Pin_15);
	   for(i=0;i<TOUCH_READ_TIMES;i++)					//²ÉÑù4´Î.
	     {
					T_CS_CLR;		//cs
					SPI_SndRecv(0xD0);
...
//	static u16 sDataX,sDataY;
  ...
  yScreen = SPI_Y();
	...
	if((xScreen>1)&&(yScreen>1)&&(xScreen<=240)&&(yScreen<=320)) {
...
int dataX_jiaozhun;
int dataY_jiaozhun;
void jiaozhun(int dataX,int dataY) {
	if(abs(dataX - 25) <= 25) {
		dataX_jiaozhun = 25;
	}
	else if(abs(dataX - 75) <= 25) {
		dataX_jiaozhun = 75;
	}
	else if(abs(dataX - 125) <= 25) {
		dataX_jiaozhun = 125;
	}
	else if(abs(dataX - 175) <= 25) {
		dataX_jiaozhun = 175;
	}
	else if(abs(dataX - 225) <= 25) {
		dataX_jiaozhun = 225;
	}
	...
	else if(abs(dataY - 125) <= 25) {
		dataY_jiaozhun = 125;
	}
	else if(abs(dataY - 175) <= 25) {
		dataY_jiaozhun = 175;
	}
	else if(abs(dataY - 225) <= 25) {
		dataY_jiaozhun = 225;
	}
	else if(abs(dataY - 275) <= 25) {
		dataY_jiaozhun = 275;
	}
...
//AI:´«4ÖÖºÚ×ÓµÄÏûÏ¢£¬½ÓÊÕ4ÖÖ°××ÓµÄÒÆ¶¯ÏûÏ¢
//vs:±¾»úÅÐ¶Ï
...
extern int sDataX_all_WHITE;
extern int sDataY_all_WHITE;
extern int dDataX_all_WHITE;
extern int dDataY_all_WHITE;
extern int cnt;
extern int recvDealFlag;
extern char recvData;
	...
	if(AI_VS_Flag == 0) {
		while(!winFlag) {				//ºóÐøÓ¦¸ÃÉ¾³ýwhile
			...
			if(moveFlag == 1) {
				...
				*/
					...
					if(sDataX_all_BLACK == 5 || sDataY_all_BLACK == 5 || dDataX_all_BLACK == 5 || dDataY_all_BLACK == 5 || chess[sDataX_all_BLACK][sDataY_all_BLACK] != 1 || chess[dDataX_all_BLACK][dDataY_all_BLACK] || abs(sDataX_all_BLACK + sDataY_all_BLACK - dDataX_all_BLACK - dDataY_all_BLACK) != 1 || abs(sDataX_all_BLACK - dDataX_all_BLACK) >= 2 || abs(sDataY_all_BLACK - dDataY_all_BLACK) >= 2) {
						moveFlag = 0;
						recvDealFlag = 0;
						continue;
					}
				...
				sDataX_all_BLACK = USART_to_LCD_X(sDataX_all_BLACK);
					...
					LCD_ClearFullColor_Circle(250 - sDataX_all_BLACK, sDataY_all_BLACK, 15);
				...
				while(USART_GetITStatus(USART1, USART_IT_TXE) != RESET) {};
					...
					if(clearFlag == 1) {
						 ...
						 deleteX = USART_to_LCD_X(deleteX);
						 deleteY = USART_to_LCD_Y(deleteY);
						 LCD_ClearFullColor_Circle(250 - deleteX, deleteY, 15);
						 while(recvDealFlag != 4) {}
						 winFlag = recvData;
		...
		while(!winFlag) {				//ºóÐøÓ¦¸ÃÉ¾³ýwhile
			...
			if(moveFlag == 1) {								//ºÚÆåÏÈ×ß
					...
					if(chess[sDataX_all][sDataY_all] != (count% 2 + 1) || chess[dDataX_all][dDataY_all] || abs(sDataX_all + sDataY_all - dDataX_all - dDataY_all) != 1 || abs(sDataX_all - dDataX_all) >= 2 || abs(sDataY_all - dDataY_all) >= 2) {
					moveFlag = 0;
					recvDealFlag = 0;
					continue;
				}
				...
				if(count % 2 == 0) {
					LCD_ClearFullColor_Circle(250 - sDataX_all, sDataY_all, 15);
