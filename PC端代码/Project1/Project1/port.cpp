	...
	char synchronizeflag)
{
	...
	TimeOuts.ReadTotalTimeoutMultiplier = 1000; //读时间系数
	TimeOuts.ReadTotalTimeoutConstant = 1000; //读时间常量			sleep
...
int WzSerialPort::receive(void* buf, int maxlen)
{
	...
	else
	{
		...
		ClearCommError(hCom, &dwErrorFlags, &comStat); //清除通讯错误，获得设备当前状态
		while(!comStat.cbInQue) { cout << "wait" << endl; Sleep(500); }; //如果输入缓冲区字节数为0，则返回false
